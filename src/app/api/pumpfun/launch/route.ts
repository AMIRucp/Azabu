...
import { prepareLaunchTransaction } from '@server/tokenLaunch';
import type { LaunchIntent } from '@server/tokenLaunch';
...
export const POST = withTradeRateLimit(async (req: NextRequest) => {
  try {
    ...
    const result = await prepareLaunchTransaction(intent, imageBuffer, imageName);
